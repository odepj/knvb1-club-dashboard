...
from client.test.mock_knvb_datacentre_request_handler import MockKnvbDatacentreRequestHandler, mock_teams_response, \
    mock_uitslagen_response
...
requestHandler = MockKnvbDatacentreRequestHandler(_BASE_URL, _AUTH_PATH, _API_KEY, _STATUS_CODES)


def mock_get_all_teams() -> list[KnvbTeamInfoDTO]:
    return requestHandler.mock_handle(url_path='/teams', data_classtype=KnvbTeamInfoDTO,
                                      mock_response=mock_teams_response())


def mock_get_uitslagen_by_team_id(team_id, weeknummer='A', comptype=None) -> list[KnvbUitslagDTO]:
    """Input parameters\n
    ...
    return requestHandler.mock_handle(url_path=url, params=params, data_classtype=KnvbUitslagDTO,
                                      mock_response=mock_uitslagen_response())
